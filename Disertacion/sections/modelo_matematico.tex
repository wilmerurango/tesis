...
\section{Primeira abordagem: modelos com demanda independente e autorizações estáticas}
...
\end{equation}
A restrição  \eqref{eq: m1_binaria_alpha} é utilizada para determinar quando a variável de decisão $X$ assume valores iguais e diferentes de zero. Assim, $\alpha$ toma o valor de 1 no primeiro caso e 0 no segundo.
\begin{equation}
...
\end{equation}
Dentro do problema, é necessário garantir que, para todos os trechos com a mesma estação de origem, os trechos mais curtos disponibilizados para venda sejam mais baratos que os trechos mais longos. Isso é estabelecido para evitar que passageiros comprem bilhetes para um trecho maior e desembarquem em estações anteriores. Essa situação é controlada pela restrição \eqref{eq: m1_preco_estacao_inicio}.
\begin{equation}
	\begin{split}
...
\section{Segunda abordagem: modelos com demanda comportamental  e autorizações estáticas} \label{sec: modelagemComportamental}
...
\section{Terceira abordagem: modelos com demanda independente e autorizações dinâmicas}
...
Para este modelo, toma-se como referência o modelo básico da abordagem estática e tipo de demanda independente, com a diferença de que agora a variável de decisão de autorizações está indexada no tempo, conforme mostrado a seguir.
...
\begin{align}
	 ...
	 & Y_{ijvkt} \geq  X_{ijvkt} + Y_{i,j,v,k + 1,t} , \quad \forall(i,j) \in OD, v \in V, k \in K_v / k < \lVert K_v \rVert , t \in T       \label{eq: rel_x_y_2}                                                                \\
	 & \sum_{(i,j) \in OD}\sum_{t\in T} Y_{i,j,v,k,t} \leq A_v, \quad  k = min\{K_v\}, \forall i \in O / (i,j) \in OD,   \forall v \in V     \label{eq: cap_min_y}                                                                \\
	 & X_{ijvkt} \leq d_{ijvkt},  \quad \forall (i,j) \in OD / i < j  ,v \in V, k \in K_v, t\in T   \tag{\ref{eq: m1_assig_menor_dem}}                                                                                            \\
	 & A_{0,v} = Q_v \quad \forall v \in V  \tag{\ref{eq: m1_ini_disponi}}                                                                                                                                                        \\
	 & X_{0,j,v,k,t} = 0 \quad \forall j \in D,\, v \in V,\, k \in K_v,\, t \in T  \tag{\ref{eq: m1_ini_assig}}                                                                                                                   \\
	 & X_{ijvkt} \in \mathbb{Z}^+ \quad \forall(i,j) \in OD,\, v \in V,\, k \in K_v,\, t \in T  \tag{\ref{eq: m1_dom_assig}}                                                                                                      \\
	 ...
	 & A_{iv} \in \mathbb{Z}^+ \quad \forall i \in O,\, v \in V  \tag{\ref{eq: m1_dom_disponi}}
\end{align}
Note que as restrições \eqref{eq: m1_autho_mayor_assig_1er_class}, \eqref{eq: m1_autho_mayor_assig_mas_autho} e
\eqref{eq: m1_cap_autho_1er_class} foram alteradas e substituídas pelas restrições \eqref{eq: rel_x_y_1}, \eqref{eq: rel_x_y_2} e  \eqref{eq: cap_min_y} . As novas restrições têm a mesma função, do tipo nesting, mas agora serão aplicadas para cada um dos períodos, e não mais para o horizonte de reserva de forma geral.
...
\section{Quarta abordagem: modelos com demanda comportamental e autorizações dinâmicas}
...
\subsection{Modelo completo comportamental-dinâmico}
O modelo \textbf{ \texttt{Comp\_Comporta\_Din}} pode ser escrito da seguinte forma:
\allowdisplaybreaks
\begin{align}
	 & Max \quad Z = \sum_{(i,j)\in OD} \sum_{v\in V} \sum_{k\in K_v} \sum_{t\in T} \sigma_{ijvk}  P_{ijvk} X_{ijvkt}     \tag{\ref{eq: fo_beha}}                                                                                 \\
	 & \text{s.a.}  \notag                                                                                                                                                                                                        \\
