...
"""
Obsercavoes:
1) Esta classe é usada para quando se adiciono o índice de tipos de assentos 
á variável A, e ficou indexada como A[i,v], pelo que esta classe cria um novo 
conjunto chamado IV.

2) Aleḿ este documento tem um almelhor estructura para a criação dos modelos

3) Foi eliminado do modelo o indice de tempo "t" da variavel "Y", e foram feitos os ajustes
em funcao dessa desicao.
"""
...
class GeralModelY:
    ...
    def create_sets_parameters(self) -> None:
        ...
        index_ijvk = self.preco[['Origin', 'Destination', 'Vagon', 'Class']].apply(tuple, axis=1)
        index_ijvk = list(dict.fromkeys(index_ijvk))
...
# =================================[-START- CREACAO DO MODELO]========================================
    ...
    def defini_variables(self) -> None:
        ...
        self.Y = self.model.addVars(self.index_ijvk, vtype=GRB.INTEGER, name="Y")
    ...
    def capacity_restric(self) -> None:
        ...
        for i in self.I:
            for v in self.V:
                ...
                # restrição .6
                self.model.addConstr(
                    quicksum(self.Y[i_,j,v_,k] for i_,j,v_,k in self.index_ijvk if (i == i_) and (v == v_) and  (k == self.preco.loc[(self.preco["Origin"]==i) & (self.preco["Destination"]==j) & (self.preco["Vagon"])]["Class"].to_list()[0]) ) <= self.A[i,v], 
                    name=f"CapY_({i},{v})"
                )

    def relation_x_y_restric(self) -> None:
        for i,j,v,k in self.index_ijvk:

            VK_ = self.preco.loc[(self.preco["Origin"]==i) & (self.preco["Destination"]==j) & (self.preco["Vagon"]==v)]["Class"].to_list()
            Tempo = self.demanda.loc[(self.demanda["Origin"]==i) & (self.demanda["Destination"]==j) & (self.demanda["Vagon"]==v) & (self.demanda["Class"]==k)]["DBD"].to_list()
            ...
            if k != last_k:
                # restrição .8
                self.model.addConstr(
                    self.Y[i,j,v,k] >=  quicksum(self.X[i,j,v,k,t] for t in Tempo) + self.Y[i,j,v,VK_[pos_k+1]],
                    name=f"RelXY1_({i},{j},{v},{k})"
                )
            else:
                # restricao .7
                self.model.addConstr(
                    self.Y[i,j,v,k] >=  quicksum(self.X[i,j,v,k,t] for t in Tempo),
                    name=f"RelXY2_({i},{j},{v},{k})"
                )
    ...
    def binary_restric(self) -> None:
        ...
        for i,j,v,k,t in self.indexCombiDem:
            ...
            if k != last_k:
                ...
                self.model.addConstr(
                    self.BL[i,j,v,k,t] == self.BX[i,j,v,k,t] - self.BX[i,j,v,VK_[pos_k+1],t],
                ...
                # [start] restrições Skiplagging
                self.model.addConstr(
                    self.BL[i,j,v,k,t] == self.BX[i,j,v,k,t],
