...
class BaseModel:
    ...
    def create_model(self): #I, J, rota, VK, NAD, BR, CR, P, Q, d, index, indexCombiDem, indexCombiDem0, demanda, dd
        ...
        A = model.addVars(rota, vtype=GRB.INTEGER , name="A") #rota = I2 (antes a rota foi nomeada de I2)
        ...
        for i,j,v,k,t in indexCombiDem:
            ...
            # restricao .9.2
            model.addConstr(
                Y[i,j,v,k,t] <= self.Q*BY[i,j,v,k,t],
...
def save_solution(model, BX, BL, P, d, X, Y, A, BY, perio, nameModel, indexCombiDem, path):
    ...
    for i, j, v, k, t in indexCombiDem:
        # if (i,j) in BR.keys():
        lista.append([i+'-'+j,i,j,v, k, t, P[i,j,v,k], d[i,j,v,k,t], A[i].X, X[i,j,v,k,t].X, Y[i,j,v,k,t].X , BY[i,j,v,k,t].X, BX[i,j,v,k,t].X, BL[i,j,v,k,t].X])
        # else:
    ...
    a.to_excel(path+'/ResultadoVars'+ nameModel +'.xlsx', index=False)
